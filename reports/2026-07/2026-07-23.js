...
window.RESEARCH_REPORTS.push({
  ...
  "topPickRationales": {
    "open-skill-risk": "优先精读它，因为它把第三方 Agent skill 明确建模为真实供应链依赖，解决的是 Agent 在安装和调用外部技能时如何识别风险的问题。它与 LLM/Agent 供应链安全直接相关，适合继续做 skill marketplace 风险基准、权限 manifest、签名发布和运行时阻断复现。",
    "license-laundering": "优先精读它，因为它把 AI 供应链中的数据集、模型和应用许可证流转变成可量化问题，补足了只关注恶意代码和投毒的安全视角。它适合发展为 AI-BOM / Model-BOM / Dataset-BOM，也适合做合规型供应链治理选题。"
  },
  "papers": [
    {
      "id": "open-skill-risk",
      ...
      "methodHighlights": "论文收集 263 个风险技能，按威胁类型分为 7 类，并为每个技能配套标准用户任务和沙箱环境；实验覆盖 3 个主流 CLI Agent 框架和 13 个前沿 LLM。相比只用合成 prompt-injection 或少量手写工具的基准，它的优势是更贴近真实 skill marketplace，并能观察模型、CLI harness 与第三方 skill 组合后的运行时行为。",
      "keyFindings": "实验显示，即使最安全的配置仍约有 17% 场景会执行不安全动作，并归纳出三类失败：未识别风险、识别风险但未阻止执行、超出用户意图范围执行技能指令。",
    ...
    },
    {
      ...
      "methodHighlights": "论文将 Hugging Face 上的数据集和模型与 GitHub 应用连接成 dataset → model → application 链路，定义两类 license laundering：无许可证 artifact 在下游获得确定标签，以及一种许可证类别在再分发中被替换为另一种。相比只检查单个仓库或单个模型卡许可证的做法，它的优势是端到端追踪 AI artifact 在数据集、模型和应用之间的传播链。",
      "keyFindings": "结果显示 62.3% 链路经过至少一个无声明许可证 artifact，且带义务许可证类别端到端保留率均低于 7%。这说明许可证义务在 AI 供应链下游传播中极易丢失。",
    ...
    },
    {
      ...
      "methodHighlights": "论文构建五 Agent CI/CD pipeline，使用来自多个提供商的生产 LLM，并在 shadow-mode LLM firewall 后执行合成实验。攻击输入是一个外部 issue，要求加入“usage telemetry”功能，实质代码会读取环境变量并外传。相比单 Agent 代码生成安全评测，它的优势是把 triage、developer、scanner、reviewer、approver/deployer 串成流水线，能观察风险如何被上游权威话术和下游流程继承。",
      "keyFindings": "论文报告权威 framing 会让下游 verifier 看见问题却仍引用预批准理由继续放行；scanner 对 laundered pull request 的通过率可达 80%，最坏实验单元 compromise 达 55%。",
    ...
    },
    {
      ...
      "methodHighlights": "论文形式化 agent reconnaissance，识别攻击者希望提取的 knowledge assets 及其如何放大间接提示注入攻击；KYA 自动探测 Agent、建立 target profile，并基于 profile 生成攻击。相比直接投放固定攻击 payload 的基准，它的优势是加入攻击前侦察阶段，更贴近真实渗透测试流程。",
      "keyFindings": "实验覆盖 agent-security benchmark 和真实 coding agent，作者报告 reconnaissance 驱动的攻击能提升间接提示注入效果，并计划释放框架、benchmark 和 baseline。",
    ...
    },
    {
      ...
      "methodHighlights": "论文把多 Agent 应用抽象为多个角色和多条内部通信通道，并将攻击面从输入边界扩展到跨 Agent 的 message passing。相比只测单模型越狱或入口过滤的基准，它的优势是关注 planner、worker、verifier、synthesizer 等角色之间的内部消息传播。",
      "keyFindings": "核心结论是：单个安全模型组合起来不必然形成安全系统；内部通道可能把恶意目标、错误约束或被污染的中间结果继续传递给后续 Agent。",
    ...
    },
    {
      ...
      "methodHighlights": "论文通过多 Agent 模拟合成多样化轨迹，训练一个包含 anticipation 与 adjudication 两个耦合任务的 guard。CoAA-RL 奖励那些能帮助下游安全判断的预测。相比只判断当前输入或当前动作是否违规的 guardrail，它的优势是面向长程轨迹预测潜在风险。",
      "keyFindings": "最终 guard 模型 Vanguard 在四个 Agent 安全基准上提升平均保护率 15.9 个百分点，同时 benign task completion 提升 5.1 个百分点。",
    ...
    },
    {
      ...
      "methodHighlights": "PRO-LONG 保存完整结构化 interaction log，并利用 coding agent 能力按需搜索历史，而不是把更多历史直接塞进上下文。相比简单长上下文拼接或固定 memory retrieval，它的优势是让 Agent 用程序化方式选择、搜索和复用历史交互记录。",
      "keyFindings": "论文在 ARC-AGI-3 public game set 上报告平均提升 18.0 个百分点，最高达到 76.1% pass@1，同时 token 使用减少 4.2–5.8 倍；相关代码和日志可用性在摘要中有说明。",
